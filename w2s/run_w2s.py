...
def run_train(cfg: SFTConfig):
    ...
    weak_preds_root = shared_root / cfg_name / "weak" / cfg.weak_model_name.split('/')[-1] / "predictions"
    ...
    train_args["output_dir"] = str(root / cfg_name / "w2s" / f"{cfg.weak_model_name.split('/')[-1]}_{cfg.strong_model_name.split('/')[-1]}")
