...
if __name__ == "__main__":
    ...
    model_dict = {'resnet':['50','101'],'resnet-d':['18','34'],'dinov2':['s14']}
    # algorithm.eval()
    ...
    for tort,loader in enumerate(eval_loaders):
        for model in ['dinov2']:
            ...
            for size in model_dict[model]:
                ...
                print('Feature Shape: ',features_torch.shape)
                labels_torch = torch.cat(true_labels,dim=0)
                inds_torch = torch.cat(inds,dim=0)

                torch.save(features_torch,os.path.join(args.output_dir, f'{tort_list[tort]}/{model}_{size}/features.pt'))
                torch.save(labels_torch,os.path.join(args.output_dir, f'{tort_list[tort]}/{model}_{size}/labels.pt'))
                torch.save(inds_torch,os.path.join(args.output_dir, f'{tort_list[tort]}/{model}_{size}/indices.pt'))
