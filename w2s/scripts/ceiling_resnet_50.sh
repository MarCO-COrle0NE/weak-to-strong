#!/bin/bash
#SBATCH --job-name=ceiling_resnet50_mse
#SBATCH --array=1-2
...
python3 -m domainbed.scripts.ceiling\
       ...
       --dataset ${DATASET}\
       --hparams "{\"lr\":$LR, \"batch_size\":256, \"input_shape\":224, \"model\": \"${STS}\", \"resnet18\":0, \"vit\":0, \"resnet50_augmix\":1, \"dinov2\":0, \"dino\":0, \"arch\": \"${INPUT_ST}\"}"\
       --seed $INPUT_SEED\
...
python3 -m domainbed.scripts.ceiling\
       --data_dir=/scratch/$USER/data/\
       --dataset ${DATASET}\
       --hparams "{\"lr\":1e-4, \"batch_size\":256, \"input_shape\":224, \"model\": \"${STS}\", \"resnet18\":0, \"vit\":0, \"resnet50_augmix\":1, \"dinov2\":0, \"dino\":0, \"arch\": \"${INPUT_ST}\"}"\
       --seed $INPUT_SEED\
       --trial_seed $INPUT_SEED\
       --start_step 6001\
       --steps 8001\
...
python3 -m domainbed.scripts.eval_mse\
       ...
       --output_dir /scratch/jl4476/${DATASET}/ceiling/${INPUT_SEED}/${STS}_${INPUT_ST}\
       --checkpoint /scratch/jl4476/${DATASET}/ceiling/${INPUT_SEED}/${STS}_${INPUT_ST}\
       --hparams "{\"lr\":1e-4, \"batch_size\":512, \"input_shape\":224, \"T\":9, \"tc_temperature\":9, \"index_dataset\":1, \"model\": \"${STS}\", \"resnet18\":0, \"vit\":0, \"resnet50_augmix\":1, \"clip\":0, \"dinov2\":0, \"dino\":0, \"arch\": \"${INPUT_ST}\"}"\
