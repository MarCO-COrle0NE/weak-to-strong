...
#from domainbed.intrinsic import modify_network_intrinsic
...
if __name__ == "__main__":
    ...
    parser.add_argument('--algorithm', type=str, default="StudentSingle")
    ...
    parser.add_argument('--subset_holdout_fraction', type=float, default=1)
    ...
    for env_i, env in enumerate(dataset):
        ...
        if env_i == 1:
            if args.indices_dir:
                dir = args.indices_dir+'/'+str(env_i)
                with open(dir+'/indices.json','r') as file:
                    class_indices = json.load(file)
                val_, in_ = misc.split_dataset_by_class(in_,1-args.subset_holdout_fraction,class_indices,misc.seed_hash(args.trial_seed, env_i))
            elif args.subset_holdout_fraction != 1:
                print("subset_holdout_fraction not used (class_indices required): "+str(args.subset_holdout_fraction))
            else:
                print("Prepared uda_split without indices_dir")
            uda_splits.append((in_, None))
    ...
    uda_loaders = [InfiniteDataLoaderWithoutReplacement(
        ...
        weights=env_weights,
        batch_size=hparams['batch_size'],
        num_workers=dataset.N_WORKERS,seed=args.seed,drop_last=False)
        for i, (env, env_weights) in enumerate(uda_splits)]

    #target_loader = FastDataLoader(dataset=dataset[args.test_envs[0]],batch_size=64,num_workers=dataset.N_WORKERS)

    # eval_loaders = [FastDataLoader(
    #     dataset=env,
    #     batch_size=64,
    #     num_workers=dataset.N_WORKERS)
    #     for env, _ in (in_splits[1:] + out_splits[1:])]
    # eval_weights = [None for _, weights in (in_splits[1:] + out_splits[1:])]
    eval_loaders = [FastDataLoader(
        dataset=env,
        batch_size=64,
        num_workers=dataset.N_WORKERS)
    ...
    eval_loader_names = ['env{}_in'.format(i)
        for i in range(1,len(in_splits))]
    ...
    if args.checkpoint:
        ...
        if args.checkpoint_algorithm in student_classes:
            algorithm = student_class(dataset.input_shape, dataset.num_classes,
                len(dataset) - len(args.test_envs), all_data, args.test_envs[0], hparams)
            ...
            if not args.dinov:
                ...
                algorithm = student_class(dataset.input_shape, dataset.num_classes,
                len(dataset) - len(args.test_envs), all_data, args.test_envs[0], hparams, featurizer = algorithm_base.encoder)
            else:
                ...
                algorithm = student_class(dataset.input_shape, dataset.num_classes,
                len(dataset) - len(args.test_envs), all_data, args.test_envs[0], hparams, featurizer = algorithm_base)
    else:
        algorithm = student_class(dataset.input_shape, dataset.num_classes,
            len(dataset) - len(args.test_envs), all_data, args.test_envs[0], hparams)
    if args.freeze:
    ...
    #     modify_network_intrinsic(algorithm,args.intrinsic_dimension,said=args.said,device=device)
    #     if "momentum" not in hparams:
    #         hparams["momentum"] = 0.0
    ...
    if algorithm_dict is not None:
        algorithm.load_state_dict(algorithm_dict,strict=args.strict)

    trainable_params = sum(p.numel() for p in algorithm.network.parameters() if p.requires_grad)
    ...
    for step in range(start_step, n_steps):
        ...
        if generalization:
            minibatches_device = [(x.to(device,non_blocking=True), y.to(device,non_blocking=True), i.to(device,non_blocking=True))
        ...
        if (step != 0) and (step % checkpoint_freq == 0) or (step == n_steps - 1):
            ...
            for name, loader, weights in evals:
                if 'env1_out' in name:
                        acc,loss_eval = misc.acc_loss(algorithm, loader, weights, device, indexed=True, mse=args.mse)
                        results[name+'_acc'] = acc
                        results[name+'_loss'] = loss_eval
                        agreement, agreement_neg, agreement_pos = misc.agreement(algorithm, loader, weights, device, indexed=True)
                        results['agreement'] = agreement
                        results['agreement_neg'] = agreement_neg
                        results['agreement_pos'] = agreement_pos
                        cluster_err = misc.clustering_error(algorithm, loader, weights, device, indexed=True)
                        results['cluster_err'] = cluster_err
                else:
                        acc = misc.accuracy(algorithm, loader, weights, device, indexed=True)
                        results[name+'_acc'] = acc
                #acc, cluster_err, loss_eval = misc.acc_cluster_loss(algorithm, loader, weights, device, indexed=True)
                #results[name+'_cluster_err'] = cluster_err
                #results[name+'_loss'] = loss_eval

            #cluster_err = misc.clustering_error(algorithm, target_loader, None, device, indexed=True)
            #cluster_err_all = misc.clustering_error_all(algorithm, eval_loaders, None, device, indexed=True)
            # results['cluster_err'] = cluster_err
            # results['cluster_err_all'] = cluster_err_all
