...
def all_accs(ft=False, twenty=False, algos=['WeightedOutput']):
    result = {'acc':{},'pgr':{}}
...
def all_accs_table(ft=False,twenty=False, algos=['WeightedOutput']):
    records = []
    for dataset in DATASETS:
        for st in ST:
            if 'b' in st:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--algos', type=str, nargs='+', default=['WeightedOutput'])
    ...
    result = all_accs(ft=args.ft,twenty=args.twenty,algos=args.algos)
    records = all_accs_table(ft=args.ft,twenty=args.twenty,algos=args.algos)
