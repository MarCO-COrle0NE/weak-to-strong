...
if __name__ == "__main__":
    ...
    if args.dataset in vars(datasets):
        dataset = vars(datasets)[args.dataset](path=args.data_dir,tokenizer_name=args.model_name,split_ratio=[args.n, args.N, args.test_length],max_length=args.max_length,seed=args.seed,flip_prob=args.flip_prob)
    else:
    ...
    if args.intrinsic_dimension:
        if args.said:
            model = intrinsic_dimension_said(model,args.intrinsic_dimension,None,set(),"fastfood", device='cpu')
        else:
            model = intrinsic_dimension(model,args.intrinsic_dimension,None,set(),"fastfood", device='cpu')
    ...
    if args.save_labels:
            ...
            #dataloaders = [DataLoader(train_dataset, batch_size=hparams['batch_size'],shuffle=False) for train_dataset in dataset]
            ...
            train_data['ground_truth_label'] = train_data['label']
            train_data['label'] = weak_labels

            # Save weak labeled data
            #os.makedirs(os.path.join(args.data_dir,f'prajjwal1'), exist_ok=True)
            #os.makedirs(os.path.join(args.data_dir,f'microsoft'), exist_ok=True)
            #os.makedirs(os.path.join(args.data_dir,f'google'), exist_ok=True)
            if args.flip_prob:
                weak_label_file = os.path.join(args.data_dir,f"{args.seed}_{args.model_name}_{args.n}_{args.flip_prob}.tsv")
            else:
                weak_label_file = os.path.join(args.data_dir,f"{args.seed}_{args.model_name}_{args.n}.tsv")
            os.makedirs(os.path.dirname(weak_label_file), exist_ok=True)
