...
fea_path_dict = {
    ...
    'resnet50': "./precomputed/ColoredMNIST/resnet50.pt", #(23705, 2048)
    'resnet101': "./precomputed/ColoredMNIST/resnet101.pt", #(23705, 2048)
    #'resnet152': "./precomputed/utkface_resnet152.pt", #(23705, 2048)
    'dinov2-s14': "./precomputed/ColoredMNIST/dinov2-s14.pt", #(23705, 768)
...
class W2S_UTKFace:
    def __init__(self, args, weak_tag='resnet18', strong_tag='resnet152', ridge=1e-6, noise=0.0, step=default_step):
        ...
        self.labels = torch.load("./precomputed/ColoredMNIST/labels.pt")#.float()
        self.noise = noise
        self.ridge = ridge
        self.step = step
        # load features
...
strong_tag = 'dinov2-s14'
weak_tags = ['resnet18', 'resnet34', 'resnet50', 'resnet101']
...
val_scal_N, val_scal_n = simulation_sample_scale_dsw(simulators, n_range=[n_min, n_max], N_range=[N_min, N_max], nstart=nstart, Nstart=Nstart, trials=14, npts=20,)

weak_str = '-'.join(weak_tags)
name = f'coloredmnist_{weak_str}_{strong_tag}_ridge{ridge}_n{n_min}-{n_max}_N{N_min}-{N_max}_nstart{nstart}_Nstart{Nstart}_pgr'
