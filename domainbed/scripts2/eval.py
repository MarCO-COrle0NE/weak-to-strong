...
if __name__ == "__main__":
    ...
    parser.add_argument('--tc_name', type=str,default='model.pkl')
    ...
    for i in range(len(dataset)):
        ...
        if args.cal_teacher:
            ...
            len(dataset) - len(args.test_envs), hparams)
        state_dict = torch.load(path+'/'+args.tc_name)
    ...
    def drop_column(tensor, col_index):
    ...
    def record_logits(teachers, dc, output_file, dataloader, batch_size=64):
        all_data = []
        correct = 0
        count = 0
        # model.eval()
        with torch.no_grad():
            for idx, (x, y) in enumerate(dataloader):
                ...
                dc_pred_list = dc_pred.tolist()
                
                if args.avg:
                    logits = outputs.mean(dim=0)
                else:
                    weights = F.softmax(drop_column(dc_pred,args.test_envs[0]),dim=-1)
                    weights = weights/(weights.sum(dim=-1).view(weights.shape[0],1))
                    #weights = F.softmax(torch.index_select(dc_pred, 1, self.weight_columns),dim=-1)
                    logits = (outputs.permute(1,0,2)*weights[:,:,np.newaxis]).sum(dim=1)
                ...
                for i in range(len(x)):
                    all_data.append([idx * batch_size + i, y_list[i]] + logits_list[i] + dc_pred_list[i] + [tc_pred[i] for tc_pred in tc_preds] + [tc_max_prob[i] for tc_max_prob in tc_max_probs])

        print('Acc: ', str(correct/count))
        # Write to CSV file
        # with open(output_file, mode='w', newline='') as file:
        #     writer = csv.writer(file)
        #     writer.writerow(['index', 'ground_truth'] + [f'logit_{i}' for i in range(len(logits_list[0]))] + [f'dc_logit_{i}' for i in range(len(dc_pred_list[0]))] + [f'tc_{i}_pred' for i in tc_keys] + [f'tc_{i}_max_prob' for i in tc_keys])
        #     writer.writerows(all_data)

    output_file = os.path.join(args.output_dir,'prob.csv')
    record_logits(teachers,dc, output_file, eval_loaders[0])
