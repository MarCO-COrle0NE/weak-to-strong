...
ST = ["s14","b14","l14"]
TC=["50"]
SUBSET = [0.02,0.05,0.07,0.1,0.2,0.5,0.75]
ALGORITHM = ['Average']
...
cols_interest_all = [f'env{i}_in_acc' for i in range(4)]+[f'env{i}_out_acc' for i in range(4)]+['epoch','loss','step'] +['agreement','agreement_neg','agreement_pos','env3_in_loss','env3_out_loss','cluster_err','cluster_err_all']
            ...
            for tc in TC:
                for algo in ALGORITHM:
                    for subset in SUBSET:
                        ...
                        path = f'{dataset}/st_dinov/{algo}_{subset}_{tc}_{st}'
                        #print(path)
