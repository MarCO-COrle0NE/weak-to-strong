...
parser.add_argument('--test_size', type=int, default=3000, help='Test set size')
...
# }
features_dict = {
    'resnet18': torch.load("./precomputed/CIFAR10/resnet18.pt"), #(23705, 512)
    'resnet34': torch.load("./precomputed/CIFAR10/resnet34.pt"), #(23705, 512)
    'resnet50': torch.load("./precomputed/CIFAR10/resnet50.pt"), #(23705, 2048) **ResNet50 is better than ResNet101**
    'resnet101': torch.load("./precomputed/CIFAR10/resnet101.pt"), #(23705, 2048)
    'dinov2': torch.load("./precomputed/CIFAR10/dinov2-s14.pt"),
    'vit_base': torch.load("./precomputed/CIFAR10/vit_base.pt"),
    'mamba_base': torch.load("./precomputed/CIFAR10/mamba_base.pt")
}
features_w = features_dict[weak_tag]
features_s = features_dict[strong_tag]
labels = torch.load("./precomputed/CIFAR10/labels.pt")#.float() #(23705,)
ridge = 1e-8
n = int(4e4)
...
fea_path_dict = {
    'resnet18': "./precomputed/CIFAR10/resnet18.pt", #(23705, 512)
    'resnet34': "./precomputed/CIFAR10/resnet34.pt", #(23705, 512)
    'resnet50': "./precomputed/CIFAR10/resnet50.pt", #(23705, 2048)
    'resnet101': "./precomputed/CIFAR10/resnet101.pt", #(23705, 2048)
    #'resnet152': "./precomputed/utkface_resnet152.pt", #(23705, 2048)
    'dinov2-s14': "./precomputed/CIFAR10/dinov2-s14.pt", #(23705, 768)
    'vit_base': "./precomputed/CIFAR10/vit_base.pt",
    'mamba_base': "./precomputed/CIFAR10/mamba_base.pt"
}
...
model_label_dict = {
    ...
    'vit_base':'VIT-base',
    'mamba_base':'MambaOut-base'
}

class W2S_UTKFace:
    def __init__(self, args, weak_tag='resnet18', strong_tag='resnet152', ridge=1e-6, noise=0.0, step=default_step):
        self.args = args
        self.weak_tag = weak_tag
        self.strong_tag = strong_tag
        self.labels = torch.load("./precomputed/CIFAR10/labels.pt")#.float()
        self.noise = noise
        self.ridge = ridge
        self.step = step
        # load features
    ...
    def run_simulation_(self, n, N, ridge=1e-6, seed=42, verbose=True, step=10,lr=1,history_size=300):
        # SFT - W2S split
    ...
    def run_simulation(self, n, N, ridge=1e-6, trials=1, verbose=True, step=10,lr=0.1,history_size=30):
        pgrs = []
        oprs = []
        er_w2ss = []
        er_ws = []
...
name = f'cifar_{weak_tag}_{strong_tag}_ridge{ridge}_n{n_min}-{n_max}_N{N_min}-{N_max}_nstart{nstart}_Nstart{Nstart}_mse'
...
strong_tag = 'mamba_base'
weak_tags = ['resnet18', 'resnet34', 'resnet50']
...
name = f'cifar_{weak_str}_{strong_tag}_ridge{ridge}_n{n_min}-{n_max}_N{N_min}-{N_max}_nstart{nstart}_Nstart{Nstart}_pgr'
