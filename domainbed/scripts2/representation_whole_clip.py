...
if __name__ == "__main__":
    ...
    for tort,loader in enumerate(eval_loaders):
                ...
                for i, (x,y,indices) in enumerate(loader):
                    images = [resize(image) for image in x]
                    x = processor(images=images,return_tensors='pt',do_rescale=False)["pixel_values"]
                    x = x.to(device)
                    with torch.no_grad():
                        outputs = vision_encoder(pixel_values=x)
