...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        ...
        for i, (x,y) in enumerate(eval_loaders[0]):
            ...
            if not args.euclidean:
                loss,info = contrastive_loss(feat1,feat2)
                loss1 = info['part1']
                loss2 = info['part2']
                test_loss1 = loss1.mean()
                test_loss2 = loss2.mean()
                loss1_list.append(test_loss1.item())
                loss2_list.append(test_loss2.item())
            else:
    ...
    if not args.euclidean:
        loss1 = sum(loss1_list) / len(test_loss_list)
        loss2 = sum(loss2_list) / len(test_loss_list)
