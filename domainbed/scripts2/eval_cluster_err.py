...
# def clustering_acc(predicted_clusters,true_labels_np):
#      # Step 3: Compute the confusion matrix
#     cm = confusion_matrix(true_labels_np, predicted_clusters)

#     # Step 4: Apply Hungarian algorithm to find the optimal cluster-label mapping
#     row_ind, col_ind = linear_sum_assignment(-cm)  # Negative sign because we want to maximize accuracy

#     # Step 5: Calculate the clustering accuracy
#     accuracy = cm[row_ind, col_ind].sum() / cm.sum()
#     return accuracy

def clustering_error(predicted_clusters, true_labels):
    if isinstance(true_labels, torch.Tensor):
        true_labels = true_labels.cpu().numpy()
    if isinstance(predicted_clusters, torch.Tensor):
    ...
    # Step 2: Find the majority vote for each cluster
    # For each predicted cluster, assign the most frequent true label
    majority_labels = np.argmax(cm, axis=0)  # Find the true label with the max count for each cluster
    
    # Step 3: Map predicted clusters to majority-voted true labels
    predicted_mapped = majority_labels[predicted_clusters]
    
    # Step 4: Calculate clustering error (misclassification rate)
    error = np.mean(predicted_mapped != true_labels)
...
if __name__ == "__main__":
    ...
    algorithm.eval()
    if args.classifier:
        cluster_err = misc.clustering_error(algorithm,eval_loaders[0],None,device,indexed=True)
        print('Clustering Err: '+str(cluster_err))
    else:
        features = []
        true_labels = []
