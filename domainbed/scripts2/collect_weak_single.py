...
ST = ["b14"]
TC=["50"]
SUBSET = [0.02,0.05,0.1,0.2,0.5,1]
#ALGORITHM = ['Average']
...
cols_interest = [f'env{i}_in_acc' for i in range(2)]+[f'env{i}_out_acc' for i in range(3)]+['epoch','loss','step'] + ['env2_out_loss','cluster_err']#+['agreement','agreement_neg','agreement_pos']
...
def select_data(file,step=None):
    ...
    model_ground_truth = df.loc[df['env1_in_acc'].idxmax()]
                                ...
                                records.append(
                                    {
                                        ...
                                        'gen_in_acc':values.env1_in_acc,
                                        'gen_out_acc':values.env1_out_acc,
