...
if __name__ == "__main__":
    ...
    if args.color:
        ...
        true_labels = []
        colors = []
        for i, (x,y,color,indices) in enumerate(eval_loaders[0]):
            x = x.to(device)
            with torch.no_grad():
        ...
        np.savez(os.path.join(args.output_dir, 'representations.npz'), representations=features_np, labels=true_labels_np, colors=colors_np, predictions=preds_np)
    else:
        features = []
        preds = []
        true_labels = []
